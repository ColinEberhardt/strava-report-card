...
module.exports.msgHandler = async (event, context) => {
  ...
  do {
    ...
    athleteData.rides = athleteData.rides.concat(
    ...
    await updateStatus(
      `Data for ${athleteData.rides.length} rides downloaded ...`
    );
    await wait(1000);
  } while (!breakLoop);

  if (athleteData.rides.length === 0) {
    await updateStatus(
  ...
  const ridesWithPhotos = athleteData.rides
  ...
  for (let i = 0; i < ridesWithPhotos.length; i++) {
    ...
    const idx = athleteData.rides.findIndex(r => r.id === ridesWithPhotos[i].id);
    athleteData.rides[idx] = activity;
    await wait(1000);
  }
